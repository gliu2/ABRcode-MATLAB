% Wrapper script to analyze all ABR files 
%
%
...
ABR_PATH = 'D:\George-abr\ABR'; % '\\Ricci-abr\d\George\ABR';
IGNORE_DATES = {'20210718', '20210714', '20210708'};

% Iterate over each experiment date
date_folders = dir(ABR_PATH);
...
for jj=1:num_dates
    ...
    % 11-20-21: if folder date is in do not analyze list, then skip
    if contains(date_subfolders(num_dates + 1 - jj).name, IGNORE_DATES)
        continue
    end
...
%     path = uigetdir();
    ...
    for j=1:length(subfolders)
        ...
        % Obtain path to average ABR or DPOAE csv file
        if IS_ABR
