...
%
% Inner products are calculated as cross-variance between a single trace ABR
% at a given dB level and the coherent average ABR at maximum dB level. 
%
% Cross-covariance is calculated with 'coeff' normalization. This is 
% cross-correlation with mean-subtracted, RMS-divided distribution, which 
% normalizes by single-trace's mean and standard deviation, respectively. 
%
% Assumptions:
...
function dist_innerprod = analyze_innerprod_ABR(X_csv, mylags, varargin)
...
%         [r, lags] = xcorr(single_trace, signal_basis, scaleopt); % r: (2*SAMPLES-1) x 1 vector. xcorr: 2nd input shifts by lag amount relative to 1st input
        [r, lags] = xcov(single_trace, signal_basis, 'coeff'); % r: (2*SAMPLES-1) x 1 vector. xcorr: 2nd input shifts by lag amount relative to 1st input
