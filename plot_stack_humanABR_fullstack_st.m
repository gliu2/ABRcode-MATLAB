% function plot_stack_humanABR_fullstack_st(varargin)
% Load and plot a stack of averaged single trace human ABRs at different
% intensities, in ascending order of intensity. FOR SUBJECTS S002 and
% after.
% 
% Filenames in input folder 'path' must have format ST02_tipv_10_C_st
% -> FIRST RUN merge_singletrace_humanABR_polarities.m on the folder to
% have .mat files with merged rarefaction (R) and condensation (C) human
% single trace ABR data. 
%
% Last edit George Liu 1-1-24
%
...
PATH_X = 'd:\users\admin\Documents\GitHub\ABRcode-MATLAB\SINGS001_tiph_x_1-22-23.mat'; % Used to get metadata about x-axis time values
% ylim_max = [-0.5, 0.5];
% ylim_max = [-500, 500];
ylim_max = [-300, 300];
% ylim_max = [-120, 120];
...
SINGLETRACE_KEY = 'blk'; % single trace (which can be condensation, rarefaction, or bulk average) are unlike bulk average which is already the average of rarefaction and condensation traces
...
%% Analyze threshold
cutoff = 1;
[threshold_D, metric_D, dist_innerprod] = get_thresh_CAPABR_D(X_csv_cache, db_levels_descend, cutoff, true);
...
fig2 = figure;
NUM_LEVELS = n_levels;
A_descending = db_levels_descend;
...
%% Save figure
save_file = [plot_title, '_humanABRstackfull'];
savefig_multiformat(gcf, SAVE_PATH, [save_file, '_thresholdpaper_Fig3_histreg'])
